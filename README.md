...
`Progress`; file-edit diffs use `DiffView`; the live activity indicator and
...
`/ide` file metadata use `Breadcrumb`; and the main footer uses `SessionStatus`
plus `ModeLine`. Menu
...
bars, connector rows, diff wrapping, framed panels, breadcrumbs, activity
shimmer, detail rows, and footer width handling therefore exercise the shared
