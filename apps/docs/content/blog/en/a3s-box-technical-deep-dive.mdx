---
title: A 40MB MicroVM Runtime Written in Rust — A Perfect Docker Replacement for AI Agent Sandboxes
description: A deep dive into A3S Box architecture — 200ms cold start, 55 Docker-like commands, hardware-level isolation, and confidential computing
date: "2026-02-19"
author: A3S Lab
tags: [box, microvm, rust, tee, docker, ai-agent]
---

> When we strip away all the technical jargon and return to the essence of computing, a core question emerges: **Can we run every workload on its own operating system kernel while maintaining container-level startup speed and developer experience?** A3S Box answers with a definitive yes — a single 40MB binary, no daemon, 200ms cold start, 55 Docker-like commands, hardware-level isolation, and optional confidential computing.
...
> 📖 For complete documentation and API reference, visit: [https://a3s-lab.github.io/a3s/](https://a3s-lab.github.io/a3s/)
...
- **Docker-compatible user experience**: 55 Docker-like commands — developers don't need to learn new tools. `a3s-box run nginx` is as simple as `docker run nginx`, but with a completely different security model underneath.
...
**a3s-box-cli (CLI Layer)**: Provides 55 Docker-like commands and is the primary interface for user interaction with the system. It translates user commands into calls to the runtime layer.
...
### 7.1 55 Docker-Like Commands

A3S Box provides 55 Docker CLI-like commands, covering all aspects of container lifecycle management. Developers can seamlessly migrate existing Docker workflows to A3S Box without modifying scripts or learning new command syntax.
...
| **Docker CLI compat** | Native | Partial | Via shimv2 | No | Yes (55 commands) |
...
| Command compat | Native | 55 Docker-like commands, consistent syntax |
...
A3S Box provides native support on macOS via Apple HVF; developers don't need to install Docker Desktop. 55 Docker-like commands make migration cost nearly zero. TSI network mode requires zero configuration, suitable for rapid iteration.
...
- **Docker-compatible user experience**: 55 commands, zero migration cost
...
>
> Documentation: [https://a3s-lab.github.io/a3s/](https://a3s-lab.github.io/a3s/) | GitHub: [https://github.com/AI45Lab/Box](https://github.com/AI45Lab/Box)
