...
description: 深入解析 A3S Box 的架构设计——200ms 冷启动、55 个 Docker 兼容命令、硬件级隔离与机密计算的技术实现，以及 v2.0.5 中可被 crictl/kubelet 直达的 CRI 运行时（核心 Pod 生命周期与 exec 已端到端打通，但尚未完全 critest 合规）
date: "2026-02-19"
author: A3S Lab
tags: [box, microvm, rust, tee, docker, ai-agent]
---

> 当我们剥离所有技术术语的外衣，回到计算的本质，一个核心问题浮现出来：**我们能否让每一个工作负载都运行在自己的操作系统内核之上，同时保持容器级别的启动速度和开发体验？** A3S Box 给出了肯定的答案——一个 40MB 的单一二进制，无守护进程，200ms 冷启动，55 个 Docker 兼容命令，硬件级隔离 + 可选机密计算。
...
- **Docker 兼容的用户体验**：55 个 Docker 兼容命令，开发者无需学习新工具。`a3s-box run nginx` 就像 `docker run nginx` 一样简单，但背后是完全不同的安全模型。
...
**a3s-box-cli（命令行层）**：提供 55 个 Docker 兼容命令，是用户与系统交互的主要界面。它将用户的命令翻译为对 runtime 层的调用。
...
### 7.1 55 个 Docker 兼容命令

A3S Box 提供了 55 个与 Docker CLI 兼容的命令，覆盖了容器生命周期管理的方方面面。开发者可以将现有的 Docker 工作流无缝迁移到 A3S Box，无需修改脚本或学习新的命令语法。
...
> **现状说明（v2.0.5，必读）**：A3S Box 的 CRI 运行时**仅支持 Linux**。在此之前它从未能被任何标准工具直达——`grpc-go >= 1.57` 发送的 socket 路径 `:authority` 是百分号编码的，上游 h2 会在任何 RPC 执行之前以 `PROTOCOL_ERROR` 重置该流。v2.0.5 通过 `third_party/h2` 中一个经由 `[patch.crates-io]` 接入的内置 h2 补丁放宽了这一校验，**CRI 服务现在可以被 `crictl`、kubelet 和 `critest` 经由其 Unix 域套接字直达**。
>
> 当前**已端到端打通**的能力：CRI v1 `RuntimeService` + `ImageService`（经 UDS）；完整的 Pod sandbox + 容器生命周期（`RunPodSandbox`/`runp` → `CreateContainer`/`create` → `StartContainer`/`start` → `ps` → `Stop` → `Remove` → `stopp` → `rmp`，一个 Pod sandbox 对应一个 Box MicroVM）；经 Kubernetes SPDY/3.1 remotecommand 协议的 **`exec`**（`kubectl exec` / `crictl exec`，TTY 与非 TTY、stdin/stdout/stderr 及退出码传播均可用）；容器 stdout/stderr 捕获到 CRI `log_path` 并可经 `crictl logs` 读取；RuntimeClass 镜像覆盖。这些已在带 `/dev/kvm` 的主机上经 `crictl` 验证。
>
> **尚不可用 / 尚不合规**（请勿假定其可用）：CRI 的 **`attach`**；以及更严格的 `critest` 合规项（精确的 CRI 日志格式、Linux `SecurityContext`、seccomp/AppArmor、命名空间共享、挂载传播、`port-forward` 等）。当前 `critest` 基线约为 80 个运行项中通过 23 个。换言之，A3S Box 的 CRI **尚不是可直接投产的 Kubernetes 运行时，也尚未完全 `critest` 合规**——核心生命周期与 exec 已端到端工作，而 attach 与完整合规仍在推进中。合规进展追踪见 box 仓库的 `docs/cri-conformance.md`。

### 18.2 规划中的架构（尚未实现，非当前能力）

> **重要提示**：本节描述的 `BoxAutoscaler` CRD、`ScaleRequest`/`ScaleResponse` 扩缩容 API、`InstanceRegistration` Gateway 自注册以及细粒度的实例生命周期状态机，均为**前瞻性/探索性设计草案，截至 v2.0.5 尚未实现**，也未出现在权威的 `crates/box/README.md` 中。请勿将其当作可用能力来依赖或集成。A3S Box 当前在 Kubernetes 方向上实际打通且被追踪的工作，仅限于 18.1 节所述的核心 Pod 生命周期与 exec；实际的合规进展与待办项以 box 仓库的 `docs/cri-conformance.md` 为准。下文保留为对未来方向的说明性草图，不代表现有实现。

设想中，A3S Box 可定义自定义资源 `BoxAutoscaler` 用于在 Kubernetes 中实现 MicroVM 的自动扩缩容，并配套 `ScaleRequest`/`ScaleResponse` 协议、实例健康上报（CPU、内存、并发请求数）以及实例启动后经 `InstanceRegistration` 向 A3S Gateway 自注册、停止时取消注册的发现机制。每个实例将经历 `Creating → Booting → Ready → Busy → Draining → Stopping → Stopped`（异常时进入 `Failed`）的状态转换。**这些均为规划，尚未落地。**
...
| **Docker CLI 兼容** | ✅ 原生                     | ⚠️ 部分           | ⚠️ 通过 shimv2           | ❌                   | ✅ 55 命令              |
| **K8s 集成**        | ✅ CRI                      | ✅ CRI              | ✅ CRI                     | ⚠️ containerd-shim | ⚠️ CRI（Linux，核心生命周期 + exec；尚未完全 critest 合规） |
...
# macOS Apple Silicon:
curl -L https://github.com/AI45Lab/Box/releases/latest/download/a3s-box-latest-macos-arm64.tar.gz | tar xz
# Linux x86_64:
curl -L https://github.com/AI45Lab/Box/releases/latest/download/a3s-box-latest-linux-x86_64.tar.gz | tar xz
./a3s-box version
# 解压即用，零依赖
```
...
A3S Box 在 macOS 上通过 Apple HVF 提供原生支持，开发者无需安装 Docker Desktop。55 个兼容命令使得迁移成本几乎为零。TSI 网络模式零配置，适合快速迭代。
...
**Kubernetes Operator**：如 18.2 节所述，`BoxAutoscaler` CRD 截至 v2.0.5 尚未实现；规划中它将从 `v1alpha1` 起步并逐步演进，增加更多自动化运维能力：
...
>
> 🔗 项目文档：[https://a3s-lab.github.io/a3s/](https://a3s-lab.github.io/a3s/) | GitHub：[https://github.com/AI45Lab/Box](https://github.com/AI45Lab/Box)
