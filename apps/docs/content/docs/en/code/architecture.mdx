...
Prompt
  ...
  -> LLM generation
  -> model-selected, delegated, direct, or programmatic tool calls
...
calls can originate from the model, a direct SDK helper, a delegated child run,
or the `program` tool; they still converge on the same registry, guards, trace
sink, and workspace services.
...
controlled `ctx` object. A program call is still a tool call: permission
policy, hook/AHP integration, timeouts, artifact limits, tracing, and workspace
capability checks remain in the execution path. By default, scripts may call
every registered tool except `program`; callers can narrow that surface with
`allowed_tools`. The docs contract verifies scripts that compose `readFile`,
`read`, `glob`, `grep`, `ls`, `bash`, `git`, and generic `tool(name, args)`,
then return one structured result to the agent.
