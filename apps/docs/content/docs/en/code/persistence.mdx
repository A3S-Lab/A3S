...
| --- | --- | --- | --- |
| Session snapshot | `session.save()` or `autoSave` | `agent.resumeSession(id, options)` | Rehydrate a conversation plus runtime evidence such as artifacts, traces, run records, verification reports, and subagent task snapshots. |
