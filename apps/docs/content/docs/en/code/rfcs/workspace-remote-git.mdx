...
2. **mTLS.** Pass a client cert + key path via the backend config; the
   underlying `reqwest::Client` is configured to present them.
...
empty bearer token; the client emits a `tracing::warn!` on construction
...
the same pattern used by [`WorkspaceVersionConflict`](../sessions) on the
S3 path.
...
shape used by `S3WorkspaceBackend` (see [Telemetry](../telemetry)):
