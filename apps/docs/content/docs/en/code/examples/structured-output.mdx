...
- `generate_object` is a built-in tool registered independently of built-in skills. The examples enable `builtinSkills` / `builtin_skills` because most real sessions also want the skill catalog, not because `generate_object` depends on it.
