...
The SDK does not expose the TUI's `/btw` command as `session.btw()`. To ask an
ephemeral side question, snapshot the current history and pass it explicitly to
`send` or `stream`. Explicit history is used for that call only; it does not
append the answer back into the session history.
