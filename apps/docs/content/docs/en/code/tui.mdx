...
inside the Use worker only after parent TUI confirmation. OCR follows the same
first-use pattern: after a read-only doctor reports missing or damaged models,
the worker requests the bounded `ocr_install` tool through parent confirmation
and continues to extraction after installation succeeds. Direct
`a3s use ocr extract` prepares the models automatically without requiring a
