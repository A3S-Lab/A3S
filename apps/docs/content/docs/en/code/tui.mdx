...
| A3S Use | [A3S-Lab/Use](https://github.com/A3S-Lab/Use) | Independently released Browser, Document, native Office, built-in OCR, optional Office compatibility, and signed external application capabilities projected into Code through standard MCP and Skills. |
...
| Application capabilities | A3S Use is prepared on first TUI use when policy allows, then projects built-in Browser, Document, native Office, OCR, optional OfficeCLI compatibility, and installed signed external domains into a dedicated `use` worker. |
...
## Browser, Document, Office, And OCR Through A3S Use
...
a3s install use/document # optional parser/shared OCR model pre-warm

# Optional OfficeCLI compatibility provider. Native Office is built into Use.
a3s install use/office

# Same shared PP-OCRv6 bundle as Document.
a3s install use/ocr
a3s use ocr doctor --json

# The first local Browser launch reuses or prepares the shared Chrome runtime.
a3s use browser open https://example.com

# Native Office needs no provider; the first compatibility command prepares OfficeCLI.
a3s use office native create report.docx --json
a3s use office get report.docx /body --json

# Inspection is native and non-installing; raster parsing uses packaged models.
a3s use document inspect report.docx --json
a3s use document parse report.docx --ocr auto --json

# A missing model bundle is prepared automatically when policy allows.
a3s use ocr extract ./scan.png --json
```

The complete verified Use archive contains Browser, Document, Office, and OCR
Skills plus PP-OCRv6 Small ONNX models. Browser and A3S Search resolve the same
managed Chrome cache; Document and OCR resolve the same model directory. Normal
official installation therefore avoids duplicate browser and OCR downloads.

When the parent Use binary is ready, Code consumes its versioned
capability registry and watches both its generation and content revision.
Provider changes and extension enable, disable, install, or upgrade events then
hot-plug their MCP and verified `SKILL.md` surfaces into the current session.
If first-use setup was disabled or failed, install the parent explicitly and
...
The primary coding model intentionally does not receive raw `mcp__use_*`
definitions. It sees a dedicated `use` worker through `task` and delegates
application work to that worker. The worker can see only `mcp__use_*`; it
cannot use shell, workspace, unrelated MCP, or recursive delegation tools.
Managed Skills provide domain guidance but cannot widen this permission
boundary. Ask naturally, for example, “Use Browser to inspect this page,”
“Use Document to recover this mixed DOCX,” “Use Office to update this
workbook,” or “Use OCR on this scan.” If a capability is unavailable, the
worker reports the typed failure instead of falling back to another tool. The
default Browser surface includes doctor and a bounded installer; missing
managed Browser files can therefore be prepared inside the Use worker only
after parent TUI confirmation. Native Office needs no provider. If an
unpromoted operation requires the optional compatibility route, the worker
requests `office_install_compat` through parent confirmation and uses the
hot-plugged route once it is ready. Document and OCR share the same first-use
model path: after a read-only doctor reports missing or damaged models, the
worker requests `document_install_ocr` or `ocr_install` through parent
confirmation and continues only after installation succeeds. Direct Browser
launches, Office compatibility commands, OCR extraction, and Document parsing
with selected raster evidence prepare their respective shared or pinned assets
without requiring a separate install command.
...
| Document | `mcp__use_document__*` | Inspects native DOCX/XLSX/PPTX structure first, then adds selected local PP-OCRv6 evidence with part, digest, model, confidence, polygon, and bounding-box provenance. Inspect and parse are read-only; only the shared model installer requires parent HITL. |
| Native Office | `mcp__use_office__*` | Uses the bounded native Office package kernel without another provider. Reads are closed-world; mutations and destructive saves require HITL. The optional compatibility installer is an open-world mutation requiring parent HITL. |
| Built-in OCR | `mcp__use_ocr__*` | Runs the pinned `PP-OCRv6_small` detection and recognition models locally through ONNX Runtime. Doctor and extraction are closed-world read-only tools; the separate idempotent installer is an open-world mutation requiring parent HITL. Source bytes never leave the device. |

Document and OCR never fall back to Tesseract, Python, PaddlePaddle, Browser,
Microsoft Office, LibreOffice, or an off-device OCR service.
