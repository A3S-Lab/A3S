...
ACL is intentionally close to HCL: a document is parsed into a list of named
blocks. Top-level bare attributes are represented as single-attribute blocks so
editing tools can keep one uniform AST shape.
...
`ParseError` carries a message plus line and column. The lexer tracks spans for
tokens, which lets editor integrations point users at the failing location.
