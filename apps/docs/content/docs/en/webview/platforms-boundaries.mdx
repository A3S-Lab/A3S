...
If `a3s-webview` is missing, `a3s code` may fall back to opening a normal system
browser. That fallback belongs to the TUI host. The helper process itself does
not discover or launch alternate browsers.
