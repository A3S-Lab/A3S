...
that is not wired to ACL in v1.0.7).
...
<Callout type="warn">
  **Defined but not populated in v1.0.7.** The collector also defines
...
<Callout type="warn">
  **v1.0.7 limitation.** Although `access_log_enabled` defaults to `true`, no
...
<Callout type="warn">
  **Outbound propagation is W3C-only.** Regardless of any propagation setting,
  ...
  injector is not wired into the request path in v1.0.7. Do not expect
