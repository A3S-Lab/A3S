...
| 对象 | 写入方 | 恢复入口 | 用途 |
| --- | --- | --- | --- |
| Session snapshot | `session.save()` 或 `autoSave` | `agent.resumeSession(id, options)` | 恢复对话，以及 artifact、trace、run record、verification report、subagent task snapshot 等运行证据。 |
...
checkpoint 时，才用 `resumeRun(runId)`。
