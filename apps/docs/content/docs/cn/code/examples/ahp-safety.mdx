...
A3S Code 会话可以通过可插拔的传输层（AHP，即 Agent Harness Protocol，
智能体驾驭协议）连接到外部 harness。无论由哪种传输承载消息，安全层都是独立的：
**安全提供器**会审查工具调用，而**权限策略**决定哪些操作可以在无人参与的情况下
执行。当你需要一个由远程宿主驱动、且工具策略仍由 A3S Code harness 显式管控的
会话时，请参考本页。
...
其他传输遵循相同的写法——`WebSocketTransport`、`UnixSocketTransport` 以及子进程
stdio 的 `StdioTransport` 在这里都可以互换。无人值守任务需要在没有匹配 allow
规则时失败关闭，可将 `defaultDecision` 设为 `'deny'`；仅在受信任的沙箱环境中才使用
`'allow'`。
