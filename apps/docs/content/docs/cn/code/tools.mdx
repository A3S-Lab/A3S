...
| Runtime builtins | `web_fetch`、`web_search`、`batch`、`program` | 由 core tool executor 注册。`batch` 和 `program` 会包住当前 registry，因此其内部可用工具仍取决于当前 session surface。 |
...
中的模型选择工具调用会经过 permission policy、confirmation、hooks/AHP、timeout、
artifact limit 与 workspace path check。`session.tool(...)` 这类 direct SDK call 是宿主控制面调用；调用 SDK 前应先在你的应用中授权。
